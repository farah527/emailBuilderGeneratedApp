...
import com.fastcode.emailtest2.commons.search.SearchFields;
...
import com.fastcode.emailtest2.addons.emailbuilder.domain.datasource.IDataSourceManager;
import com.fastcode.emailtest2.addons.emailbuilder.domain.datasource.IDataSourceMetaManager;
...
import com.fastcode.emailtest2.addons.emailbuilder.domain.model.QDataSourceEntity;
...
public class DataSourceAppService {
	...
	public FindDataSourceByIdOutput findById(Long eid) {
		...
		List<DataSourceMetaInput> dataSourceMeta = dataSourceMapper.dataSourceMetaEntityToDataSourceMetaInput(dataSourceList);
	...
	public BooleanBuilder searchSpecificProperty(QDataSourceEntity email,List<String> list,String value,String operator)  {
		BooleanBuilder builder = new BooleanBuilder();
	...
	public BooleanBuilder searchKeyValuePair(QDataSourceEntity email, Map<String,SearchFields> map) {
		...
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			...
			if(details.getKey().replace("%20","").trim().equals("emailTemplate")) {
				if(details.getValue().getOperator().equals("contains")) {
					builder.and(email.emailTemplate.templateName.likeIgnoreCase("%"+ details.getValue().getSearchValue() + "%"));
				}
				else if(details.getValue().getOperator().equals("equals")) {
